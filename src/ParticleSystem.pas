...
//		FSmoothing: string;
	...
	public
		...
		procedure Render(const ADestCanvas: TCanvas; const AScale: Single = 1);
...
uses
	System.SysUtils, System.Math.Vectors, FMX.Types, FMX.Canvas.GPU;
...
procedure TParticleSystem.Render(const ADestCanvas: TCanvas;
	...
	boundsRect: TRectF;
	srcPoint: TPointF;
	saveModulate: TAlphaColor;

	begin
	if  FNumParticles = 0 then
		Exit;

	saveModulate:= TAlphaColorRec.White;
...
//			tmpBmp.ReplaceOpaqueColor(FParticles[i].color);
			...
			if  ADestCanvas.InheritsFrom(TCustomCanvasGpu) then
				begin
				tmpBmp:= FTexture;
				saveModulate:= TCustomCanvasGpu(ADestCanvas).ModulateColor;
				TCustomCanvasGpu(ADestCanvas).ModulateColor:= FParticles[i].Color;
				end
			else
				tmpBmp:= FCache.GetTextureForColor(FParticles[i].Color);

			blitMatrix:=
					TMatrix.CreateScaling(AScale, AScale) *
			...
			ADestCanvas.DrawBitmap(tmpBmp, srcRect, srcRect, FParticles[i].alpha, False);

			if  ADestCanvas.InheritsFrom(TCustomCanvasGpu) then
				TCustomCanvasGpu(ADestCanvas).ModulateColor:= saveModulate;
			end;
